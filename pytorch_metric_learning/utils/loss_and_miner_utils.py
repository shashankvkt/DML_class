...
def get_all_pairs_indices(labels, ref_labels=None):
    ...
    if ref_labels is None:
        ref_labels = labels
    # makes labels -> column vector
    labels1 = labels.unsqueeze(1)
    # makes ref_labels -> row vector
    labels2 = ref_labels.unsqueeze(0)
    # matches -> square matrix, 
    # in which each row corresponds to each sample
    # and each element of the row is equal to 1 if
    # the corresponding (to element) sample has the same class
    # (like a confusion matrix)
    matches = (labels1 == labels2).byte()
    # diffs -> exactly the opposite of matches,
    # it has 1 where positives of the corresponding
    # sample exist, 0 where negatives exist
    diffs = matches ^ 1
    if ref_labels is labels:
        matches.fill_diagonal_(0)
    # a1_idx corresponds to the index of 
    # an anchor that has a positive sample inside the batch
    # p_idx corresponds to the index of
    # this aforementioned positive sample
    # for example: a1_index=tensor([2, 7]) and p_idx=tensor([35, 18])
    # means that the 3rd (having index 2) sample of batch
    # has a positive the 36th (having index 35) sample of batch
    a1_idx, p_idx = torch.where(matches)
    # exactly the same as before but this time for negatives
    # for example: a2_idx=tensor([0, 0, 0]) and n_idx=tensor([1, 2, 3])
    # means that the 1st sample of batch has as negatives the 2nd, 3rd, 4th
    # samples of batch
...
def convert_to_neg_pairs_with_unique_labels(indices_tuple, labels):
    _,_, a, n = convert_to_pairs(indices_tuple, labels)
    _, unique_idx = np.unique(labels[a].cpu().numpy(), return_index=True)
    return a[unique_idx], n[unique_idx]
